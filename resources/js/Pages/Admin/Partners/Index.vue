...
const openEditModal = (partner) => {
    ...
    form.image = null; // Don't bind the old image to file input
    form._method = 'POST'; // We use POST for file uploads in Inertia, with _method=PUT handled later if needed, but actually in Laravel with files, we should use POST and append _method manually if we use FormData, but Inertia's useForm handles POST with files. Wait, updating with file in Laravel requires POST and `_method: 'PUT'`.
    form._method = 'PUT';
    isModalOpen.value = true;
};
...
const submit = () => {
    if (isEditing.value) {
        // Inertia has a quirk with PUT and File uploads in Laravel. We MUST use POST and add _method: 'PUT'
        form._method = 'PUT';
        form.post(route('admin.partners.update', form.id), {
            preserveScroll: true,
            forceFormData: true,
...
<template>
    <Head title="Quản lý Đối tác" />

    <div class="page-header">
        <div>
            <h1 class="title">Quản lý Đối tác</h1>
            <p class="subtitle">Quản lý logo và liên kết đối tác hiển thị trên website.</p>
        </div>
            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="(partner, index) in partners.data" :key="partner.id">
                        <td>{{ (partners.current_page - 1) * partners.per_page + index + 1 }}</td>
                        <td>
                            <img v-if="partner.image" :src="'/' + partner.image" class="thumb-img" alt="Logo" />
                        </td>
                        <td class="font-medium text-dark">{{ partner.name || '---' }}</td>
                        <td>
                            <a v-if="partner.link" :href="partner.link" target="_blank" class="text-blue">{{ partner.link }}</a>
                            <span v-else>---</span>
                        </td>
                        <td class="text-right">
                            <div class="actions">
                                <button @click="openEditModal(partner)" class="btn-icon text-blue" title="Sửa">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                </button>
                                <button @click="deletePartner(partner.id)" class="btn-icon text-red" title="Xóa">
                    ...
                    <tr v-if="partners.data.length === 0">
                        <td colspan="5" class="text-center py-4 text-gray">Không có đối tác nào.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        
        <div class="pagination-wrap" v-if="partners.links && partners.links.length > 3">
    ...
    <div class="center-modal" :class="{ show: isModalOpen }">
        <div class="modal-header">
            <h2>{{ isEditing ? 'Cập nhật Đối tác' : 'Thêm Đối tác mới' }}</h2>
        ...
        <div class="modal-body">
            <form @submit.prevent="submit">
                ...
                <div class="form-group">
                    <label>Logo / Hình ảnh <span class="required" v-if="!isEditing">*</span></label>
                    <input type="file" @change="handleImageChange" class="form-control" accept="image/*" :required="!isEditing" />
                    <span class="error" v-if="form.errors.image">{{ form.errors.image }}</span>
                    <p class="text-sm text-gray mt-1" v-if="isEditing">Để trống nếu không muốn thay đổi ảnh.</p>
                </div>

                <div class="form-group">
                    <label>Liên kết (Link website đối tác)</label>
                    <input type="url" v-model="form.link" class="form-control" placeholder="https://..." />
...
.thumb-img { max-height: 40px; border-radius: 6px; border: 1px solid #e2e8f0; background: #fff; padding: 2px; }
